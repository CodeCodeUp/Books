...
- **TF-IDF内容推荐**: 基于1502维特征向量的语义相似性推荐，F1-Score 0.27%
- **三层混合推荐**: CF + TF-IDF + 兴趣加权，动态比例(50%-90%)自适应调节 ✨
- **图书详情动态推荐**: 物品CF + 内容特征，动态比例(40%-85%)自适应调节 ✨ 新增
- **智能策略路由**: 根据用户/图书状态自动选择10种推荐策略之一（个人推荐6种+图书详情4种）
- **性能优化**: 候选预筛选，计算时间从30分钟→几秒钟
- **算法评估**: 留存验证法科学评估，相比基线提升387%

#### 用户管理
- **用户注册**: 自动登录机制(返回JWT token)
- **资料完善**: 两步式引导流程
  - 步骤1: 填写年龄信息
  - 步骤2: 选择阅读兴趣(31个图书主题,Apple风格3D气泡选择器)
- **用户登录**: 密码加密验证(BCrypt)
- **个人信息管理**: 昵称、年龄编辑
- **兴趣管理**: 可随时修改阅读兴趣偏好
- **登录状态保持**: JWT token + localStorage
...
- 最新图书展示(基于出版年份)
- 图书详情查看
- **相似图书推荐**: 动态混合推荐（物品协同过滤 + 内容特征，40%-85%自适应比例） ✨ 新增
...
- 跨域配置
- 完整JWT认证
- **Apple风格UI设计**: 蓝白简约配色(#5ac8fa),清爽专业
...
图书表(books): 271,360条(含theme_id主题分类)
评分表(ratings): 433,671条 + 新评分
图书主题表(book_theme): 31个主题分类
用户兴趣表(user_interests): 记录用户选择的兴趣主题
收藏表(favorites): 0条
...
- [x] 物品协同过滤算法实现
- [x] TF-IDF内容特征提取算法
- [x] 三层混合推荐策略(动态比例 + 兴趣加权) - 个人推荐页
- [x] 图书详情页动态混合推荐(数据质量自适应) - 图书详情页 ✨ 新增
- [x] 智能策略路由(10种自适应策略：个人推荐6种+图书详情4种)
- [x] 算法评估和性能优化
- [x] 缓存机制和异步预计算
