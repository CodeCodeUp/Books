...
CREATE TABLE users (
    ...
    location VARCHAR(255) DEFAULT NULL COMMENT '地理位置（已停用，仅保留历史数据）',
    age INT DEFAULT NULL COMMENT '年龄',
    age_group VARCHAR(20) DEFAULT NULL COMMENT '年龄分组（Under 18, 18-24, 25-34, 35-44, 45-54, 55+，用于内容特征推荐）',
    country VARCHAR(100) DEFAULT NULL COMMENT '国家（用于内容特征推荐，前端动态加载已有国家列表）',
...
) ENGINE=InnoDB AUTO_INCREMENT=1000000 DEFAULT CHARSET=utf8mb4 COMMENT='用户评分表';
```
...
### 数据库版本管理
**位置**: `book-recommendation-backend/src/main/resources/db/`

```
db/
├── V1.0_Initial_Schema.sql      # 初始表结构创建
├── V1.1_Remove_Foreign_Keys.sql # 删除外键+自增ID设置
└── README.md                    # 版本管理说明
```
...
### 设计变更说明
- **移除外键约束**: 简化数据库结构，提高开发效率
- **用户表自增主键**: user_id从300001开始自动递增  
- **评分表自增主键**: rating_id从1000000开始自动递增
- **保持索引优化**: 保留所有性能优化索引
- **专业版本管理**: 后端项目集成SQL版本控制

### 最终数据库状态
```
用户表(users): 77,806条 + 新注册用户(从300001开始)
图书表(books): 271,360条
评分表(ratings): 433,671条 + 新评分(从1000000开始)
收藏表(favorites): 0条
