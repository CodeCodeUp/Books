...
public class UserController {
    ...
    @PostMapping("/register")      // 用户注册 (返回JWT token实现自动登录) ✅
    @PostMapping("/login")         // 用户登录 ✅
    @GetMapping("/{userId}")       // 获取用户信息 ✅
    @PutMapping("/{userId}")       // 更新用户信息 (nickname, age) ✅
    @GetMapping("/{userId}/interests")  // 获取用户兴趣列表 ✅
    @PostMapping("/{userId}/interests") // 保存用户兴趣 ✅
}
```
...
public class CategoryController {
    ...
    @GetMapping                    // 获取所有图书主题分类 ✅
    @GetMapping("/{themeId}")      // 获取单个主题详情 ✅
}
```

**主题数据**: 31个图书主题,包含中英文名称和emoji图标
...
#### 用户认证流程 ✅
- **登录页面** (Login.vue): 用户名/密码登录，表单验证，JWT token管理
- **注册页面** (Register.vue): 用户注册，自动登录，无需手动登录
- **完善资料页面** (CompleteProfile.vue): 两步式资料完善
  - **步骤1**: 填写年龄信息
  - **步骤2**: 选择阅读兴趣(使用InterestSelector组件)
- **个人中心** (Profile.vue): 用户信息展示和编辑，兴趣管理
- **路由守卫**: 登录状态验证，自动跳转

#### 图书展示界面 ✅
- **首页** (Home.vue): 热门图书和最新图书展示
- **图书列表** (BookList.vue): 分页展示，关键词搜索
- **图书详情** (BookDetail.vue): 图书信息展示，评分功能预留
- **推荐页面** (Recommendations.vue): 个性化推荐结果展示
- **响应式设计**: 适配不同屏幕尺寸

#### 核心组件 ✅
- **InterestSelector**: Apple风格的兴趣选择组件
  - 3D浮动气泡效果(圆形布局)
  - 智能网格布局(2×2、3×2、3×3自适应)
  - 分页展示(每页8个主题)
  - 随机选择和全选功能
  - 蓝白简约配色方案
  - 持续浮动动画效果
...
**UI设计风格**: 采用Apple风格的蓝白简约设计，主色调为浅蓝色(#5ac8fa)，清爽专业
...
### 3. 三层混合推荐算法 ✅ ✨ 核心创新（个人推荐页）

**应用场景**: 个人推荐页面的个性化推荐
**核心思想**: CF + TF-IDF + 兴趣加权的三层融合推荐

**算法特色**:
- **动态比例**: 协同过滤占比50%-90%动态调节（基于用户协同过滤置信度）
- **兴趣加权**: 匹配用户兴趣的图书评分提升30%（显式偏好优先）
- **智能路由**: 根据用户状态自动选择6种推荐策略之一

**个人推荐策略矩阵**:

| 用户状态 | 推荐算法 | 算法标识符 | 实现方法 |
|:---|:---|:---|:---|
| **有评分+有兴趣** | **三层混合** | `triple_hybrid_with_interest_boost` | `_get_triple_hybrid_recommendations()` |
| 有评分+有年龄 | 双层混合 | `dual_hybrid_cf_tfidf` | `_get_dual_hybrid_recommendations()` |
| 有评分+无特征 | 双层混合 | `dual_hybrid_cf_tfidf` | `_get_dual_hybrid_recommendations()` |
| 无评分+有兴趣 | 兴趣主题推荐 | `interest_based` | `_recommend_by_interests()` |
| 无评分+有年龄 | 年龄偏好推荐 | `age_based` | `_recommend_by_age()` |
| 无评分+无特征 | 热门优质图书 | `top_quality_books` | `_get_top_quality_books()` |

**三层混合算法流程**:
```python
1. 获取协同过滤推荐 (CF) → Top 20
2. 获取TF-IDF内容推荐 (Content) → Top 20
3. 动态计算混合比例:
   confidence = (num_similar_users / 50) × avg_similarity
   cf_ratio = 0.5 + 0.4 × confidence  # [0.5, 0.9]
4. 兴趣加权提升:
   if book.theme_id in user.interests:
       TF-IDF similarity *= 1.3  # 提升30%
       CF predicted_rating *= 1.15  # 提升15%
5. 按动态比例混合并返回 Top-N
```
...
### 4. 图书详情页动态混合推荐 ✅ ✨ 新增（图书详情页）

**应用场景**: 图书详情页的"喜欢这本书的用户也喜欢"相似图书推荐
**核心思想**: 基于图书数据质量动态调整物品CF和内容特征的混合比例

**算法特色**:
- **数据质量自适应**: 协同过滤占比40%-85%动态调节（基于图书评分数量和相似图书质量）
- **冷门书优化**: 评分少的冷门书依赖内容特征（Content占60%）
- **热门书深挖**: 评分多的热门书协同过滤权重高（CF占85%）
- **智能降级**: 4种策略自动降级处理

**图书详情推荐策略矩阵**:

| 数据状态 | 推荐算法 | 算法标识符 | CF占比 | Content占比 |
|:---|:---|:---|:---|:---|
| **CF有+Content有** | **动态混合** | `dynamic_item_cf_content` | 40%-85% | 15%-60% |
| 仅CF有 | 纯协同过滤 | `item_cf_only` | 100% | 0% |
| 仅Content有 | 纯内容特征 | `content_only` | 0% | 100% |
| 两者都无 | 同作者降级 | `same_author_fallback` | 0% | 0% |
...
**实际案例对比**:
- **超热门书** (5000评分, 相似度0.8): CF 78% + Content 22%
- **热门书** (800评分, 相似度0.7): CF 68% + Content 32%
- **中等热度** (150评分, 相似度0.6): CF 50% + Content 50%
- **冷门书** (8评分, 相似度0.5): CF 40% + Content 60%

**关键创新点**:
- **数据质量自适应**: 根据图书评分数和相似图书质量自动调整
- **冷热图书兼顾**: 冷门书内容特征占主导，热门书协同过滤更可靠
- **统一动态框架**: 与个人推荐的动态策略形成统一的自适应架构
...
**API接口设计**:
- `POST /api/recommend/user-based` - 混合推荐（个人推荐页）
  - 根据用户状态自动选择6种策略之一
  - 返回推荐结果包含动态比例元数据
- `POST /api/recommend/similar-items` - 相似图书混合推荐（图书详情页）
  - 根据图书数据质量动态调整CF和Content比例
  - 返回推荐结果包含置信度信息
...
| 算法名称 | Precision@10 | Recall@10 | F1-Score@10 | 性能评价 |
|---------|-------|-----|--------|----------|
| 热门推荐 | 30% | 4% | 8% | 基线算法 |
| 内容特征推荐 | 71% | 22% | 27% | 冷启动解决方案 |
| 用户协同过滤 | 94% | 29% | **39%** | ⭐ 最佳算法 |
| 混合推荐策略 | [7:3组合] | [智能降级] | [覆盖全场景] | 完整解决方案 |
...
**算法服务性能**:
- **数据处理**: 43万评分+27万图书+7万用户
- **响应时间**:
  - 个人推荐：缓存命中<1秒，首次计算<30秒
  - 图书详情推荐：<5秒（含动态比例计算）
- **内存使用**: 避免大型矩阵，稳定在<500MB
- **并发支持**: 多用户同时推荐请求

**推荐质量指标**:
- **个性化程度**:
  - 个人推荐：基于用户行为+兴趣的深度个性化
  - 图书详情：基于图书评分模式和内容特征的相似性推荐
- **推荐多样性**: Coverage指标显示良好的覆盖率
- **实时响应**: 评分后异步更新推荐结果
- **自适应能力**:
  - 个人推荐：协同过滤比例50%-90%动态调节
  - 图书详情：协同过滤比例40%-85%动态调节
