...
用户特征 = {
    ...
    'country': '国家',           # 字符串字段，从数据库动态获取已有国家列表
}

特征权重分配:
- 年龄组匹配: 50% (基于标准年龄分组的图书年代偏好)
- 国家匹配: 30% (文化背景和语言偏好)  
- 图书质量: 20% (评分和受欢迎程度)
```
...
def recommend_by_user_features(self, user_info, user_id, top_n):
    ...
    candidate_books = quality_books[~book_id.isin(user_rated_books)]
    ...
    for book in candidate_books:
        ...
        elif user_age_group == '25-34':
            age_score = 1.0 if book_year >= 1990 else 0.8  # 青年偏好当代文学
        elif user_age_group == '35-44':
            age_score = 1.0 if book_year >= 1980 else 0.9  # 中年偏好成熟作品
        elif user_age_group == '45-54':
            age_score = 1.0 if book_year >= 1970 else 0.9  # 偏好经典文学
        elif user_age_group == '55+':
            age_score = 1.0 if book_year <= 1990 else 0.8  # 老年偏好传统经典
        match_score += 0.5 * age_score
        
        # 2. 国家文化匹配 (30%)
        if user_country in ['usa', 'united states', 'canada', 'uk', 'australia']:
            # 英语国家用户偏好英语作者
            if english_author_names in book_author:
                match_score += 0.3 * 0.8
        elif user_country in ['germany', 'france', 'spain', 'italy']:
            # 欧洲国家用户偏好欧洲主题
            if european_keywords in book_title:
                match_score += 0.3 * 0.8
        
        # 3. 图书质量匹配 (20%)
        quality_score = (book_avg_rating / 5.0 + min(1.0, rating_count / 100)) / 2
        match_score += 0.2 * quality_score
